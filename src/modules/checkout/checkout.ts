...
} from '../promotion/promotion.interface';
...
export class Checkout {
  ...
  calculate() {
    this.finalAmount = this.products.reduce(
      (acc, product) => acc.add(product.price),
      new Decimal(0),
    );
  ...
  calculateDealDiscount(product: Product, dealRule: IDealPricingRule) {
    const buyingQuantity = this.products.filter(
      (p) => p.id === product.id,
    ).length;
  ...
  calculateFixedPriceDiscount(
    product: Product,
    fixedPriceRule: IFixedPricePricingRule,
  ) {
    const buyingQuantity = this.products.filter(
      (p) => p.id === product.id,
    ).length;
